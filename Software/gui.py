...
class ManualPage(tk.Frame):
    ...
    def start_and_collect(self,controller,sampling_rate):
        ...
        if goodToGo and int(self.sitesentry.get()) > 0:
            ...
            while sw.wells_used < self.sites.get():
    ...
    def setOrigin(self,controller):
        ...
        upButton = tk.Button(window, text = "UP",font = controller.buttonFont,bg = "white",command = lambda: self.moveOneUp(controller), repeatdelay = 500, repeatinterval = 100, height = 5, width = 10)
        downButton = tk.Button(window, text = "DOWN",font = controller.buttonFont,bg = "white",command = lambda: self.moveOneDown(controller), repeatdelay = 500, repeatinterval = 100, height = 5, width = 10)
        rightButton = tk.Button(window, text = "RIGHT",font = controller.buttonFont,bg = "white",command = lambda: self.moveOneRight(controller), repeatdelay = 500, repeatinterval = 100, height = 5, width = 10)
        leftButton = tk.Button(window, text = "LEFT",font = controller.buttonFont,bg = "white",command = lambda: self.moveOneLeft(controller), repeatdelay = 500, repeatinterval = 100, height = 5, width = 10)
...
class AutomaticPage(tk.Frame):
    ...
    def run_recipe(self,controller):
        ...
        exitbutton = tk.Button(ap,text = "Exit/Abort",font = controller.buttonFont,command = combine_funcs(controller.myPump.stop_all,ap.destroy,ap.grab_release,controller.negative_run_status))

        ap.info = tk.StringVar()
        infolabel = tk.Label(ap, anchor='w')
        infolabel['textvariable'] = ap.info
        ...
        for i in range(len(self.steplist)):
            ...
            #ap.direction = self.steplist[i].directioncombo.get()
